...
Componente que recebe dois vetores de entrada de múltiplos _bits_, quantidade de _bits_ ajustável de acordo com a necessidade, e recebe um seletor de 1 _bit_, e dependendo do valor do seletor, um dos valores de entrada passa a ser valor de saída. Esse componente está ilustrado na Figura 10.
...
Componente que recebe dois vetores de 32 _bit_ e devolve três flags, uma que indica se ambos os vetores são iguais, outra que indica se o vetor de entrada primário é menor do que o secundário, e a última indica se o vetor primário é maior ou igual ao secundário. Esse componente está ilustrado na Figura 15.
...
**Figura 16** - Diagrama de Portas do Componente RV32I Deslocador da ULA
...
**Figura 19** - Diagrama de Portas do Componente RV32I Unidade de Controle da ULA
...
**Figura 20** - Diagrama de Portas do Componente RV32I Banco de Registradores
...
têm sinal, e três flags que indicam se o vetor de dados 1 é maior, igual ou menor ao vetor de dados 2. O componente também recebe a função da instrução, para saber se o resultado da comparação gera condição de desvio, e isso é indicado pelo sinal de saída. Esse componente está ilustrado na Figura 21.

![alt text](../public/images/reference/report_components/rv32i_branch_controller.drawio.svg)
**Figura 21** - Diagrama de Portas do Componente RV32I Controlador de Desvio
...
para resolver um _hazzard_ de dado. Esse componente está ilustrado na Figura 22.

**Figura 22** - Diagrama de Portas do Componente de CPU Unidade de Encaminhamento de Dados para Execução
...
#### 3.1.14 Extensor de _bits_ para STORE (CPU)

Componente que recebe um vetor de 32 _bits_ do Banco de Registradores e, dependendo de qual instrução do tipo STORE estiver sendo usada, devolve na saída ou o vetor inteiro, ou parte dele com extensão de zeros. Esse componente está ilustrado na Figura 23.

**Figura 23** - Diagrama de Portas do Componente de CPU Extensor de _bits_ para STORE
...
#### 3.1.15 Extensor de Sinal para LOAD (CPU)

Componente que recebe um vetor de 32 _bits_ da memória RAM e, dependendo de qual instrução do tipo LOAD estiver sendo usada, devolve na saída ou o vetor inteiro, ou parte dele com extensão de zeros ou extensão de sinal. Esse componente está ilustrado na Figura 24.

**Figura 24** - Diagrama de Portas do Componente de CPU Extensor de Sinal para LOAD
...
Módulo que implementa a Unidade de Controle da ULA. Esse módulo está ilustrado na Figura 25.

**Figura 25** - Diagrama de Portas do Módulo Controlador da Unidade de Execução
...
Módulo que implementa a ULA. Esse módulo está ilustrado na Figura 26.

**Figura 26** - Diagrama de Portas do Módulo Unidade de Execução
...
Módulo que implementa o Contador de Programa, que é um Registrador Genérico. Esse módulo está ilustrado na Figura 28.

**Figura 28** - Diagrama de Portas do Módulo Contador de Programa
...
Módulo que implementa o Banco de Registradores. Esse módulo está ilustrado na Figura 29.

**Figura 29** - Diagrama de Portas do Módulo Banco de Registradores
...
Módulo que implementa o multiplexador de escrita no banco de registradores. Esse módulo está ilustrado na Figura 30.

**Figura 30** - Diagrama de Portas do Módulo Escrita no Retorno
...
Módulo que implementa o comparador. Esse módulo está ilustrado na Figura 31.

**Figura 31** - Diagrama de Portas do Módulo Comparação para Desvio
...
Módulo que implementa a lógica que determina o endereço de destino de uma instrução de desvio. Esse módulo está ilustrado na Figura 32.

**Figura 32** - Diagrama de Portas do Módulo Desvio
...
A Etapa de Decodificação de Instrução é responsável por interpretar o que será executado e definir os sinais de controle. Nesta etapa, foram implementados os módulos Unidade de Controle, Desvio, Banco de Registradores e Comparação para Desvio. Essa etapa está ilustrada na Figura 34.

**Figura 34** - Diagrama de Portas da Etapa ID
...
A Etapa de Execução é onde se realizam as operações do processador. Nesta etapa, foram implementados os módulos Controlador da Unidade de Execução, Unidade de Execução e Unidade Encaminhamento de Dados para Execução. Essa etapa está ilustrada na Figura 35.

**Figura 35** - Diagrama de Portas da Etapa EX
...
A Etapa de Escrita no Retorno é onde o resultado da operação do processador ou o valor acessado na memória RAM sai para ser escrito no Banco de Registradores. É nesta etapa que se implementa o módulo Escrita no Retorno. Essa etapa está ilustrada na Figura 37.

**Figura 37** - Diagrama de Portas da Etapa WB
