...
No Github Actions foi desenvolvido um ambiente de testes para a verificação de cada módulo VHDL. Através dele é possível realizar os testes de sínteses, unitários, stress e a compilação do _Quartus_ com as etapas de _synthesis_, _elaboration_, _fitter_ e _timing analysis_. Sendo possível ver os _logs_ e as etapas, na página do GitHub Actions, como mostra a Figura 13.

<figure style="text-align: left;">
    <img src="/images/quartus_actions.png" width="400" style="margin: 0 auto;">
    <figcaption>
        <strong>Figura 13</strong> - Logs Quartus.
    </figcaption>
</figure>


Os testes unitários e os testes de stress são realizados separadamente, mas em conjunto com o teste de síntese, sendo que o teste de síntese é realizado antes deles. Os testes unitários são realizados após cada _push_ no repositório, na pasta _/src_, sendo possível ver os _logs_ e as etapas, na página do GitHub Actions, como mostra a Figura 14.

<figure style="text-align: left;">
    <img src="/images/test_cases_actions.png" width="400" style="margin: 0 auto;">
    <figcaption>
        <strong>Figura 14</strong> - Casos de testes no Github Actions.
    </figcaption>
</figure>


Enquanto os testes de stress são realizados somente ao clicar no botão _Run workflow_ na página do GitHub Actions, como mostra a Figura 15

<figure style="text-align: left;">
    <img src="/images/button_stress.png" width="800" style="margin: 0 auto;">
    <figcaption>
        <strong>Figura 15</strong> - Botão para a execução dos testes de stress.
    </figcaption>
</figure>


No ambiente de desenvolvimento é possível executar os testes de síntese, unitários e stress, através do menu lateral de teste do VSCode, como mostra a Figura 16. 

<figure style="text-align: left;">
    <img src="/images/tests_vscode.png" width="800" style="margin: 0 auto;">
    <figcaption>
        <strong>Figura 16</strong> - Menu de testes do VSCode.
    </figcaption>
</figure>


No ambiente de desenvolvimento, também é possível executar os testes do _Quartus_ através de botões localizados na parte inferior da tela, bem como é possível abrir a interface gráfica do _Quartus_ , como mostra a Figura 17.

<figure style="text-align: left;">
    <img src="/images/buttons_quartus.png" width="800" style="margin: 0 auto;">
    <figcaption>
        <strong>Figura 17</strong> - Menu de testes do VSCode.
    </figcaption>
</figure>

Através do Quartus foi possível obter a frequência máxima do processador que ficou em 67.77 MHz, com uma área de 2985 ALMs, ocupando 16% da FPGA. Para efeito de comparação o projeto _VexRiscv_ (SPINALHDL, 2024), um projeto de RISC-V desenvolvido para FPGA utilizando _SpinalHDL_, a sua versão com a menor frequência máxima possui 124 MHz, enquanto a com maior 194 MHz. Enquanto para a área, a versão com menos ALMs possui 352 ALMs e a versão com mais possui 1764.
...
Para um teste na FPGA paras demonstrar o funcionamento do processador, foi adicionado um registrador conectado aos LEDs da placa, que é atualizado quando ocorre a escrita em um determinado endereço de memória.

Também foi realizada a documentação do projeto em um website cujo link está nas referências (DIAS, L. F.; RUGGIERO, G. V.;
SEIXAS, T. V., 2024) e que está ilustrado na Figura 18, hospedado no GitHub Pages utilizando de VitePress, uma
...
**Figura 18** - Página inicial do site de documentação (DIAS, L. F.; RUGGIERO, G. V.; SEIXAS, T. V., 2024)
...
Componente que contém a lógica necessária para realizar soma _bit_ a _bit_ entre dois vetores de dados. Esse componente está ilustrado na Figura 19

![alt text](/images/reference/report_components/generic_carry_lookahead.drawio.svg)
**Figura 19** - Topologia do Componente Genérico _Carry_ _Lookahead_
...
devolvendo na saída o resultado. Esse componente está ilustrado na Figura 20.

![Somador Genérico](/images/reference/report_components/generic_adder.drawio.svg)
**Figura 20** - Topologia do Componente Genérico Somador
...
saída. Esse componente está ilustrado na Figura 21.

![MUX 2x1](/images/reference/report_components/generic_mux_2x1.drawio.svg)
**Figura 21** - Topologia do Componente Genérico Multiplexador de duas entradas
...
**Figura 22** - Topologia do Componente Genérico Multiplexador de quatro entradas
...
**Figura 23** - Topologia do Componente Genérico Memória ROM
...
**Figura 24** - Topologia do Componente Genérico Memória RAM
...
**Figura 25** - Topologia do Componente Genérico Registrador
...
**Figura 26** - Topologia do Componente Genérico Comparador
...
**Figura 27** - Topologia do Componente RV32I Deslocador da ULA
...
_bits_ para decidir qual operação será realizada. Esse componente está ilustrado na Figura 28.

![alt text](/images/reference/report_components/rv32i_alu.drawio.svg)
**Figura 28** - Topologia do Componente RV32I Unidade Lógica e Aritmética
...
valor de _carry_,  _overflow_ e slt. Esse componente está ilustrado na Figura 29.

![alt text](/images/reference/report_components/rv32i_alu_bit.drawio.svg)
**Figura 29** - Topologia do Subcomponente RV32I ULA _bit_
...
componente está ilustrado na Figura 30.

![alt text](/images/reference/report_components/rv32i_alu_controller.drawio.svg)
**Figura 30** - Topologia do Componente RV32I Unidade de Controle da ULA
...
nos registradores apontados pelos outros dois endereços. Esse componente está ilustrado na Figura 31.

![alt text](/images/reference/report_components/rv32i_register_file.drawio.svg)
**Figura 31** - Topologia do Componente RV32I Banco de Registradores
...
de saída. Esse componente está ilustrado na Figura 32.

![alt text](/images/reference/report_components/rv32i_branch_controller.drawio.svg)
**Figura 32** - Topologia do Componente RV32I Controlador de Desvio
...
Módulo que implementa a Unidade de Controle da ULA. Esse módulo está ilustrado na Figura 33.

![Módulo Controlador da Unidade de Execução](/images/reference/report_components/module_execution_unit_controller.drawio.svg)
**Figura 33** - Topologia do Módulo Controlador da Unidade de Execução

#### 3.2.2 Módulo Unidade de Execução

Módulo que implementa a ULA. Esse módulo está ilustrado na Figura 34.

![Módulo Unidade de Execução](/images/reference/report_components/module_execution_unit.drawio.svg)
**Figura 34** - Topologia do Módulo Unidade de Execução
...
**Figura 35** - Topologia do Módulo Unidade de Controle

#### 3.2.4 Módulo Contador de Programa

Módulo que implementa o Contador de Programa, que é um Registrador Genérico. Esse módulo está ilustrado na Figura 36.

![Módulo Contador de Programa](/images/reference/report_components/module_program_counter.drawio.svg)
**Figura 36** - Topologia do Módulo Contador de Programa

#### 3.2.5 Módulo Banco de Registradores
	
Módulo que implementa o Banco de Registradores. Esse módulo está ilustrado na Figura 37.

![Módulo Banco de Registradores](/images/reference/report_components/module_register_file.drawio.svg)
**Figura 37** - Topologia do Módulo Banco de Registradores

#### 3.2.6 Módulo Escrita no Retorno

Módulo que implementa o multiplexador de escrita no banco de registradores. Esse módulo está ilustrado na Figura 38.

![Módulo Escrita no Retorno](/images/reference/report_components/module_write_back.drawio.svg)
**Figura 38** - Topologia do Módulo Escrita no Retorno

#### 3.2.7 Módulo Comparação para Desvio

Módulo que implementa o comparador. Esse módulo está ilustrado na Figura 39.

![Módulo Comparação para Desvio](/images/reference/report_components/module_branch_compare_unit.drawio.svg)
**Figura 39** - Topologia do Módulo Comparação para Desvio

#### 3.2.8 Módulo Desvio

Módulo que implementa a lógica que determina o endereço de destino de uma instrução de desvio. 
Esse módulo está ilustrado na Figura 40.

![Módulo Desvio](/images/reference/report_components/module_branch_unit.drawio.svg)
**Figura 40** - Topologia do Módulo Desvio
...
![Etapa IF](/images/reference/report_components/cpu_stage_if.drawio.svg)
**Figura 41** - Topologia da Etapa IF
...
Essa etapa está ilustrada na Figura 42.

![Etapa ID](/images/reference/report_components/cpu_stage_id.drawio.svg)
**Figura 42** - Topologia da Etapa ID
...
![Etapa EX](/images/reference/report_components/cpu_stage_ex.drawio.svg)
**Figura 43** - Topologia da Etapa EX
...
Extensor de Sinal para LOAD implementados. Essa etapa está ilustrada na Figura 44.

![Etapa MEM](/images/reference/report_components/cpu_stage_mem.drawio.svg)
**Figura 44** - Topologia da Etapa MEM
...
![Etapa WB](/images/reference/report_components/cpu_stage_wb.drawio.svg)
**Figura 45** - Topologia da Etapa WB

#### 3.3.6 Unidade de Encaminhamento de Dados para Execução (CPU)

Componente que encaminha dados das etapas MEM e WB (ver Seção 2) para a etapa EX (ver Seção 2), 
para resolver um _hazard_ de dado. Esse componente está ilustrado na Figura 46.

![alt text](/images/reference/report_components/cpu_execution_forwarding_unit.drawio.svg)
**Figura 46** - Topologia do Componente de CPU Unidade de Encaminhamento de Dados para Execução

#### 3.3.7 Unidade de Encaminhamento de Dados para Desvio (CPU)

Componente que encaminha dados da etapa MEM (ver Seção 2) para a etapa ID (ver Seção 2), para resolver um _hazard_ de
dado. Esse componente está ilustrado na Figura 47.

![alt text](/images/reference/report_components/cpu_branch_forwarding_unit.drawio.svg)
**Figura 47** - Topologia do Componente de CPU Unidade de Encaminhamento de Dados para Desvio

#### 3.3.8 Unidade de Controle de Hazard (CPU)
...
**Figura 48** - Topologia do Componente de CPU Unidade de Controle de Hazard
